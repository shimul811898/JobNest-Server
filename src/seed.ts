...
const jobs = [
  ...
  },
  {
    ...
    responsibilities: [
      ...
      'Maintain code files cleanly and collaborate via Git repository',
...
const seedDB = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/jobnest');
    ...
    const jobsWithPoster = jobs.map(job => ({
      ...job,
      postedBy: admin._id
    }));

    await Job.create(jobsWithPoster);
    console.log(`Seed: Created ${jobs.length} job listings`);
    ...
    console.log('   Email:    user@jobnest.com');
